...
const GuardianViewAppointment = () => {
  ...
  useEffect(() => {
    ...
    const fetchAppointmentDetails = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`http://capstone-production-bd9d.up.railway.app/api/get-appointments/${appointmentId}`, {
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this appointment?')) {
      try {
        await axios.delete(`http://capstone-production-bd9d.up.railway.app/api/appointments/${appointmentId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
